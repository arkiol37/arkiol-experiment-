...
export async function provisionPlanChange(
  ...
  await auditSvc.log({
    orgId,
    actorId: opts.auditActor ?? 'billing-webhook',
    action: `billing.plan_provisioned.${opts.provider}`,
    targetId: orgId,
    metadata: { planKey, provider: opts.provider, subscriptionId: opts.subscriptionId },
  });
}
...
export async function provisionCancellation(
  ...
  await auditSvc.log({
    ...
    targetId: orgId,
    metadata: { provider: opts.provider, subscriptionId: opts.subscriptionId },
  });
}
