...
export function createCronJobs(prisma: PrismaClient) {
  ...
  async function gracePeriodDowngrade(): Promise<{ downgraded: number }> {
    const expired = await prisma.org.findMany({
      where: {
        ...
        subscriptionStatus: { in: ['PAST_DUE' as any, 'UNPAID' as any] },
      },
      select: { id: true },
    });
  ...
  async function autoRefillRun(stripe: any, sendEmail?: any): Promise<{ triggered: number; failed: number }> {
