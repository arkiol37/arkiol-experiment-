...
export function inferLayoutStrategy(intent: Readonly<Intent>): StageResult<LayoutStrategy> {
  ...
  try {
    ...
    if (!parsed.success) {
      return stageResult(LAYOUT_DEFAULTS, { fallback: true, errors: parsed.error.errors.map(e => (e instanceof Error ? e.message : String(e))) });
    }

    return stageResult(parsed.data, { durationMs: Date.now() - t0 });
  } catch (err: unknown) {
    return stageResult(LAYOUT_DEFAULTS, { ok: false, fallback: true, errors: [(err instanceof Error ? err.message : String(err))], durationMs: Date.now() - t0 });
  }
}
...
export function planVariations(
  ...
  try {
    ...
    if (!parsed.success) {
      return stageResult(VARIATION_DEFAULTS, { fallback: true, errors: parsed.error.errors.map(e => (e instanceof Error ? e.message : String(e))) });
    }
    return stageResult(parsed.data, { durationMs: Date.now() - t0 });
  } catch (err: unknown) {
    return stageResult(VARIATION_DEFAULTS, { ok: false, fallback: true, errors: [(err instanceof Error ? err.message : String(err))], durationMs: Date.now() - t0 });
  }
}
...
export function modelAudience(intent: Readonly<Intent>): StageResult<AudienceProfile> {
  ...
  try {
    ...
    if (!parsed.success) {
      return stageResult(AUDIENCE_DEFAULTS, { fallback: true, errors: parsed.error.errors.map(e => (e instanceof Error ? e.message : String(e))) });
    }
    return stageResult(parsed.data, { durationMs: Date.now() - t0 });
  } catch (err: unknown) {
    return stageResult(AUDIENCE_DEFAULTS, { ok: false, fallback: true, errors: [(err instanceof Error ? err.message : String(err))], durationMs: Date.now() - t0 });
  }
}
...
export function optimizeDensity(
  ...
  try {
    ...
    if (!parsed.success) {
      return stageResult(DENSITY_DEFAULTS, { fallback: true, errors: parsed.error.errors.map(e => (e instanceof Error ? e.message : String(e))) });
    }
    return stageResult(parsed.data, { durationMs: Date.now() - t0 });
  } catch (err: unknown) {
    return stageResult(DENSITY_DEFAULTS, { ok: false, fallback: true, errors: [(err instanceof Error ? err.message : String(err))], durationMs: Date.now() - t0 });
  }
}
...
export function extractBrandSignals(brandKit: Record<string, unknown> | null): StageResult<BrandSignals> {
  ...
  try {
    ...
    if (!parsed.success) {
      return stageResult(BRAND_SIGNALS_DEFAULTS, { fallback: true, errors: parsed.error.errors.map(e => (e instanceof Error ? e.message : String(e))) });
    }
    return stageResult(parsed.data, { durationMs: Date.now() - t0 });
  } catch (err: unknown) {
    return stageResult(BRAND_SIGNALS_DEFAULTS, { ok: false, fallback: true, errors: [(err instanceof Error ? err.message : String(err))], durationMs: Date.now() - t0 });
  }
}
