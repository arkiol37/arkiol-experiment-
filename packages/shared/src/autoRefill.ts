...
export async function processAutoRefill(deps: AutoRefillDeps): Promise<{ triggered: number; failed: number }> {
  ...
  for (const org of candidates) {
    ...
    const alreadyDone = await prisma.creditTransaction.findFirst({
      where: {
        orgId: org.id,
        type: 'topup',
    ...
    try {
      ...
      if (intent.status === 'succeeded') {
        ...
        await prisma.auditLog.create({
          data: {
            ...
            metadata: toJsonValue({ packId, credits: pack.credits, paymentIntentId: intent.id, trigger: 'auto_refill' }),
          },
        });
        triggered++;
      ...
      failed++;
      console.error(`[auto-refill] org ${org.id} FAILED: ${(err instanceof Error ? err.message : String(err))}`);
...
async function getDefaultPaymentMethod(stripe: Stripe, customerId: string): Promise<string> {
  ...
  if (defaultPM && typeof defaultPM === 'object' && 'id' in defaultPM) return String((defaultPM as { id: string }).id);
