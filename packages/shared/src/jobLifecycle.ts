...
export function createJobLifecycleService(prisma: PrismaClient) {
  ...
  async function countRunning(orgId: string): Promise<number> {
    return prisma.job.count({
      where: { orgId, status: { in: ['QUEUED' as any, 'RUNNING' as any, 'PENDING' as any] } },
    });
  }
  ...
  async function countTodayVideoJobs(orgId: string): Promise<number> {
    ...
    return prisma.job.count({
      where: {
        orgId,
        type: { in: [...VIDEO_JOB_TYPES] as any[] },
  ...
  async function enqueueJob(params: {
    ...
    if (!check.allowed) {
      ...
      err.code       = check.code;
      err.statusCode = check.httpStatus;
      // Audit concurrency blocks (Task #9)
      if ((check as any).code === 'CONCURRENCY_LIMIT') {
        await auditLog.logJobEvent({
          orgId, actorId: userId, action: 'job.concurrency_blocked',
          jobId: 'pending', jobType: type, reason: check.reason,
        });
      }
      throw err;
    }
  ...
  async function markRunning(jobId: string) {
    return prisma.job.update({ where: { id: jobId }, data: { status: 'RUNNING' as any, startedAt: new Date() } });
  }
  ...
  async function markFailed(jobId: string, errorMessage: string) {
    ...
    if (job.creditDeducted && !job.creditRefunded && job.creditCost > 0) {
      ...
      if (reason) {
        ...
        await prisma.job.update({ where: { id: jobId }, data: { creditRefunded: true, status: 'REFUNDED' as any } });
  ...
  async function cancelJob(jobId: string): Promise<void> {
    ...
    await prisma.job.update({ where: { id: jobId }, data: { status: 'CANCELED' as any, canceledAt: new Date() } });
...
// ── Helpers ────────────────────────────────────────────────────────────────
function idempotencyKeyFor(job: any): string {
