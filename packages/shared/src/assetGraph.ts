...
export async function recordAssetRelationships(
  ...
  try {
    ...
    deps.logger?.warn({ err: e.message }, '[asset-graph] recordAssetRelationships failed (non-fatal)');
  }
}
...
export function buildAssetRelationships(opts: {
  ...
  if (opts.campaignId) {
    rels.push({ orgId, fromId: assetId, fromType: 'generated_asset', toId: opts.campaignId, toType: 'campaign', relationship: 'part_of_campaign', weight: 1.0 });
    rels.push({ orgId, fromId: assetId, fromType: 'generated_asset', toId: opts.campaignId, toType: 'campaign', relationship: 'belongs_to', weight: 1.0 });
  }
  if (opts.brandId) {
    rels.push({ orgId, fromId: assetId, fromType: 'generated_asset', toId: opts.brandId, toType: 'brand', relationship: 'references_brand', weight: 1.0 });
  }
  if (opts.templateId) {
    rels.push({ orgId, fromId: assetId, fromType: 'generated_asset', toId: opts.templateId, toType: 'template', relationship: 'uses_template', weight: 1.0 });
  }
  if (opts.presetId) {
    rels.push({ orgId, fromId: assetId, fromType: 'generated_asset', toId: opts.presetId, toType: 'preset', relationship: 'uses_preset', weight: 1.0 });
  }
  if (opts.archetypeId) {
    rels.push({ orgId, fromId: assetId, fromType: 'generated_asset', toId: opts.archetypeId, toType: 'archetype', relationship: 'uses_archetype', weight: 1.0 });
  }
  if (opts.explorationRunId) {
    rels.push({ orgId, fromId: assetId, fromType: 'exploration_candidate', toId: opts.explorationRunId, toType: 'exploration_run', relationship: 'exploration_of', weight: 1.0 });
  }
  if (opts.jobId) {
    rels.push({ orgId, fromId: assetId, fromType: 'generated_asset', toId: opts.jobId, toType: 'generated_asset', relationship: 'produced_by_job', weight: 1.0 });
  }
  return rels;
}
...
export async function getAssetLineage(
  ...
  try {
    ...
    const edges: GraphEdge[] = rawEdges.map((e: any) => ({
      ...
      weight:       e.weight,
      recordedAt:   e.recordedAt?.toISOString?.() ?? new Date().toISOString(),
    }));
    ...
    return {
      ...
      templateId:       templateEdge?.toId  ?? meta.templateId   ?? null,
      presetId:         presetEdge?.toId    ?? meta.presetId     ?? null,
      archetypeId:      archetypeEdge?.toId ?? meta.archetypeId  ?? null,
      explorationRunId: exploreEdge?.toId   ?? meta.explorationRunId ?? null,
    ...
    deps.logger?.warn({ err: e.message, assetId }, '[asset-graph] getAssetLineage failed');
    return null;
  }
}
...
export async function getExplorationLineage(
  ...
  try {
    ...
    const selected   = candidates.find((c: any) => c.accepted === true)?.id ?? null;
    const rejected   = candidates.filter((c: any) => c.accepted === false).map((c: any) => c.id);
    const scores     = candidates.map((c: any) => c.score ?? 0).filter((s: number) => s > 0);
    ...
    deps.logger?.warn({ err: e.message, runId }, '[asset-graph] getExplorationLineage failed');
    return null;
  }
}
...
export async function getBrandCoverage(
  ...
  try {
    ...
    const dominantPresets = (presetEdges ?? []).map((r: any) => r.toId).filter(Boolean);
    return {
      orgId, brandId,
      totalAssets:    assetCount   ?? 0,
      campaignCount:  campaignCount ?? 0,
      latestAssetAt:  latestAsset?.generatedAt?.toISOString?.() ?? null,
      dominantPresets,
      explorationRuns: exploreCount ?? 0,
    };
  } catch (e: any) {
    deps.logger?.warn({ err: e.message, orgId, brandId }, '[asset-graph] getBrandCoverage failed');
    return null;
  }
}
  ...
  try {
    ...
    return related.map((a: any) => {
      ...
      return {
        ...
        layoutFamily:     a.layoutFamily ?? null,
        stylePreset:      a.stylePreset ?? null,
        similarityHash:   a.similarityHash ?? null,
        generatedAt:      a.generatedAt?.toISOString?.() ?? new Date().toISOString(),
        edges:            [], // lazy — not loaded for bulk results
      };
    });
  } catch (e: any) {
    deps.logger?.warn({ err: e.message, assetId }, '[asset-graph] findRelatedAssets failed');
    return [];
  }
}
...
export async function getCampaignAssetSummary(
  ...
  try {
    ...
    for (const r of styleAgg ?? []) { if (r.stylePreset) styleBreakdown[r.stylePreset] = r._count.stylePreset; }
    return {
      campaignId,
    ...
    deps.logger?.warn({ err: e.message, campaignId }, '[asset-graph] getCampaignAssetSummary failed');
    return null;
  }
}
