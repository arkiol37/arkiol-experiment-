...
export function createCreditService(prisma: PrismaClient) {
  ...
  async function _updateBalance(tx: any, orgId: string, delta: number) {
    await tx.org.update({
      where: { id: orgId },
  ...
  async function grantDailyCredits(orgId: string): Promise<number> {
    ...
    const monthlySum = await (prisma as any).creditTransaction.aggregate({
      where: { orgId, type: 'daily_grant', createdAt: { gte: monthStart } },
      _sum: { amount: true },
    });
    if ((monthlySum._sum.amount ?? 0) >= plan.freeMonthlyCapCredits) return 0;
    ...
    await prisma.$transaction(async (tx: any) => {
      await tx.creditTransaction.create({
        data: {
          ...
          idempotencyKey: key, metadata: { note: params.reasonNote },
        },
      });
      ...
      await tx.auditLog.create({
        data: {
          ...
          targetType: 'credits', metadata: { amount: params.amount, note: params.reasonNote },
        },
      });
    });
  }
  ...
  async function reconcileBalance(orgId: string): Promise<number> {
    const result = await (prisma as any).$queryRaw<[{ total: bigint }]>`
...
export class InsufficientCreditsError extends Error {
  constructor(required: number, available: number, bucket: 'main' | 'daily' = 'main') {
    ...
    (this as any).code = 'INSUFFICIENT_CREDITS';
    (this as any).statusCode = 402;
  }
}
