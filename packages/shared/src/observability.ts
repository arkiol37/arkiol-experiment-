...
// Built-in checks
export function buildDbHealthCheck(prisma: { $queryRaw: (...args: unknown[]) => Promise<unknown> }): HealthCheck {
  return async () => {
    const t = Date.now();
    try {
      ...
      return { name: 'database', status: 'unhealthy', latencyMs: Date.now() - t, message: (e instanceof Error ? e.message : String(e)) };
    }
  };
}

export function buildRedisHealthCheck(getRedis: () => { ping: () => Promise<string> }): HealthCheck {
  return async () => {
    const t = Date.now();
    try {
      ...
      return { name: 'redis', status: 'unhealthy', latencyMs: Date.now() - t, message: (e instanceof Error ? e.message : String(e)) };
    }
  };
}
...
export function buildDlqHealthCheck(
  ...
  return async () => {
    ...
    try {
      ...
      return { name: 'dlq', status: 'degraded', latencyMs: Date.now() - t, message: (e instanceof Error ? e.message : String(e)) };
    }
  };
}
