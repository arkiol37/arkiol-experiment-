...
export function registerEngine(contract: Record<string, unknown>): void {
  if (_locked) {
    throw new RegistryViolationError('REGISTRY_LOCKED', `Cannot register "${String(contract.name ?? 'unknown')}" — registry is locked.`);
  }
  const parsed = EngineContractSchema.safeParse(contract);
  if (!parsed.success) {
    const issues = parsed.error.errors.map((e: {path: (string|number)[]; message: string}) => `${e.path.join('.')}: ${(e instanceof Error ? e.message : String(e))}`).join('; ');
    throw new RegistryViolationError('CONTRACT_SCHEMA_INVALID', `Invalid contract for "${String(contract.name ?? 'unknown')}": ${issues}`);
  }
...
export function validateRegistryIntegrity(): EngineValidationRecord[] {
  ...
  for (const contract of _registry.values()) {
    ...
    records.push({
      name:        String(contract.name ?? 'unknown'),
    ...
    if (violations.length > 0) {
      throw new RegistryViolationError(
        'INTEGRITY_VIOLATION',
        `Engine "${String(contract.name ?? 'unknown')}@${contract.version}" integrity failed: ${violations.join('; ')}`
      );
    }
  }
  _bootValidated = true;
