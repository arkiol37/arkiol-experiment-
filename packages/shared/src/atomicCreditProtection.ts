...
export async function holdCredits(
  ...
  try {
    ...
    logger?.error?.({ err: e.message, orgId, jobId, amount }, '[atomic-credits] holdCredits FAILED');
...
export async function finalizeCredits(
  ...
  try {
    ...
    logger?.error?.({ err: e.message, orgId, jobId, amount }, '[atomic-credits] finalizeCredits FAILED — manual review required');
    return { finalized: false, chargedAmount: 0, idempotencyKey: chargeKey, alreadyDone: false };
  }
}
...
export async function refundCredits(
  ...
  try {
    ...
    logger?.error?.({ err: e.message, orgId, jobId }, '[atomic-credits] refundCredits FAILED — manual review required');
    return { refunded: false, refundedAmount: 0, idempotencyKey: refundKey, alreadyDone: false };
  }
}
...
export async function refundBatchCredits(
  ...
  try {
    ...
    logger?.error?.({ err: e.message, orgId, batchId }, '[atomic-credits] refundBatchCredits FAILED');
  }
...
export async function computeCreditAuditSnapshot(
  ...
  try {
    ...
    const ledgerSum   = ledger?._sum?.amount ?? 0;
