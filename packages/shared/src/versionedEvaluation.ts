...
// ── Prisma JSON helpers ──────────────────────────────────────────────────────
// Prisma Json fields accept Prisma.InputJsonValue on create/update.
// These helpers convert validated Zod output into Prisma-safe JSON values
// while preserving the actual data shape (arrays stay arrays, objects stay objects).
// The round-trip through JSON.parse(JSON.stringify(...)) guarantees the output
// is a plain JSON tree that satisfies Prisma.InputJsonValue without unsafe casts.

/** Convert a validated array of plain objects to a Prisma-safe JSON array. */
function toJsonArray(arr: readonly Record<string, unknown>[]): Prisma.InputJsonValue {
  return JSON.parse(JSON.stringify(arr)) as Prisma.InputJsonValue;
}

/** Convert a validated plain object to a Prisma-safe JSON object. */
function toJsonObject(obj: Record<string, unknown>): Prisma.InputJsonValue {
  ...
  try {
    ...
    await deps.prisma.aIBenchmarkRecord?.create?.({
      data: {
        ...
        stageBreakdown:  toJsonArray(d.stageExecutions),
        stagePerfs:      toJsonArray(d.stageExecutions),
        pipelineMs:      d.totalPipelineMs,
        anyFallback:     d.anyFallback,
        fallbackCount:   d.fallbackCount,
        totalCostUsd:    d.totalCostUsd,
        abVariants:      toJsonObject(d.abVariants),
        userSelected:    d.userSelected,
        userSelectedAt:  d.userSelectedAt ? new Date(d.userSelectedAt) : null,
        userExported:    d.userExported,
        routingDecisions: toJsonObject(d.routingDecisions),
        overallScore:    d.quality.overallScore,
        brandAlignment:  d.quality.brandAlignment,
        hierarchyScore:  d.quality.hierarchyIntegrity,
        densityScore:    d.quality.densityFit,
        contrastScore:   d.quality.contrastCompliance,
        violationCount:  0,
        stylePreset:     '',
        outputFormat:    d.format,
        layoutFamily:    '',
        renderedAt:      new Date(d.evaluatedAt),
      },
    });
  } catch (e: unknown) {
    deps.logger?.warn({ err: (e instanceof Error ? e.message : String(e)), jobId: record.jobId }, '[eval] writeEvaluationRecord failed (non-fatal)');
  }
}
...
export async function recordUserSelection(
  ...
  try {
    ...
    deps.logger?.warn({ err: (e instanceof Error ? e.message : String(e)), assetId }, '[eval] recordUserSelection failed (non-fatal)');
  }
}
...
export async function recordUserExport(
  ...
  try {
    ...
    deps.logger?.warn({ err: (e instanceof Error ? e.message : String(e)), assetId }, '[eval] recordUserExport failed (non-fatal)');
  }
}
...
export async function getEngineBenchmarkSummaries(
  ...
  try {
    ...
    const records = await deps.prisma.aIBenchmarkRecord?.findMany?.({
      where:   { orgId, renderedAt: { gte: since } },
      select:  { stageBreakdown: true, overallScore: true, renderedAt: true },
    ...
    for (const rec of records) {
      const rawStages = Array.isArray(rec.stageBreakdown) ? rec.stageBreakdown : [];
      const recScore = typeof rec.overallScore === 'number' ? rec.overallScore : null;
      for (const raw of rawStages) {
        const parsed = StageExecutionRecordSchema.safeParse(raw);
        ...
        if (recScore !== null) {
          entry.qualityScores.push(recScore);
        }
      }
    }
    ...
    deps.logger?.warn({ err: (e instanceof Error ? e.message : String(e)) }, '[eval] getEngineBenchmarkSummaries failed');
    return [];
  }
}
...
export async function getRoutingModeBenchmarks(
  ...
  try {
    ...
    for (const row of agg) {
      const mode = String(row.routingMode ?? 'unknown');
      ...
      const [selectedCount, exportedCount, fallbackCount] = await Promise.all([
        deps.prisma.aIBenchmarkRecord?.count?.({
          where: { orgId, routingMode: mode, userSelected: true, renderedAt: { gte: since } },
        }).catch(() => 0),
        deps.prisma.aIBenchmarkRecord?.count?.({
          where: { orgId, routingMode: mode, userExported: true, renderedAt: { gte: since } },
        }).catch(() => 0),
        deps.prisma.aIBenchmarkRecord?.count?.({
          where: { orgId, routingMode: mode, anyFallback: true, renderedAt: { gte: since } },
        }).catch(() => 0),
      ]);
      ...
      results.push({
        mode,
        sampleCount:       count,
        avgPipelineMs:     Math.round(row._avg?.pipelineMs ?? 0),
    ...
    deps.logger?.warn({ err: (e instanceof Error ? e.message : String(e)) }, '[eval] getRoutingModeBenchmarks failed');
    return [];
  }
}
