...
export async function executeControlPlane(
  ...
  try {
    ...
    const code        = pipelineErr.code ?? 'PIPELINE_ERROR';
    ...
    log?.error?.({ jobId: request.jobId, err: pipelineErr.message, code, failureClass: fc },
      '[control-plane] Intelligence pipeline failed');

    // Transition job to appropriate failure state
    if (isPermanentFailure(code)) {
      await crashSafety.transitionJob(request.jobId, 'failed', { errorMessage: pipelineErr.message, errorCode: code });
    } else if (retryable) {
      const nextRetryAt = new Date(Date.now() + (retryDelay ?? 2000));
      await crashSafety.transitionJob(request.jobId, 'retrying', { errorMessage: pipelineErr.message, errorCode: code, nextRetryAt });
    } else {
      await crashSafety.sendToDeadLetter(request.jobId, code, pipelineErr.message, { attemptCount: request.attemptNumber });
    }
    ...
    return {
      ...
      error:       { message: pipelineErr.message, code, failureClass: fc, retryable, retryDelayMs: retryDelay },
    };
  }
  ...
  try {
    ...
    log?.warn?.({ err: e?.message, jobId: request.jobId }, '[control-plane] Checkpoint save failed (non-fatal)');
  }
...
export async function recordSelection(
  ...
  await Promise.allSettled([
    ...
    writeUserTasteSignal({ orgId, userId, stylePreset, accepted:true, sessionId, recordedAt: new Date().toISOString() }, deps),
