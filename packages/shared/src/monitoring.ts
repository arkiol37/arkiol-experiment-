...
function envNum(key: string, def: number): number {
  try {
    const v = (getEnv() as Record<string, unknown>)[key] as string | undefined;
    if (!v) return def;
...
export interface AlertEmitterDeps {
  ...
  logger?: { warn: (...a: unknown[]) => void; error: (...a: unknown[]) => void; info: (...a: unknown[]) => void };
}
...
async function emitAlert(alert: { type: string; severity: string; title: string; message: string; orgId?: string; jobId?: string; value?: number; threshold?: number; metadata?: Record<string, unknown>; resolvedAt?: string }): Promise<void> {
  const full = {
    ...alert,
  ...
  if (full.severity === 'critical') {
    ...
    _deps.onCritical?.(full as unknown as Alert);
  ...
  try {
    await _deps.onAlert?.(full as unknown as Alert);
  } catch (err: unknown) {
    log.error({ err: (err instanceof Error ? err.message : String(err)) }, '[monitoring] onAlert handler threw (non-fatal)');
  }
}
