...
export function createCrashSafetyService(deps: CrashSafetyDeps) {
  ...
  async function saveCheckpoint(cp: Omit<Checkpoint, 'idempotencyKey'> & { idempotencyKey?: string }): Promise<boolean> {
    ...
    try {
      ...
      logger?.warn({ err: e.message, jobId: d.jobId, stage: d.stage }, '[crash-safety] saveCheckpoint failed (non-fatal)');
      return false;
    }
  }
  ...
  async function recoverFromCheckpoint(jobId: string): Promise<{
    ...
    try {
      ...
      if (!row) return null;
      return {
        stageOutputs:    (row.stageOutputs ?? {}) as Record<string, unknown>,
      ...
      logger?.warn({ err: e.message, jobId }, '[crash-safety] recoverFromCheckpoint failed');
      return null;
    }
  }
  ...
  async function transitionJob(
    ...
    try {
      ...
      logger?.warn({ err: e.message, jobId, to }, '[crash-safety] transitionJob failed');
      return false;
    }
  }
  ...
  async function protectCredits(
    ...
    try {
      ...
      if (!job.creditDeducted || (job.creditCost ?? 0) <= 0) {
        ...
        await prisma.job?.update?.({ where: { id: jobId }, data: { status: 'REFUNDED' as any } });
        return true;
      }
      ...
      await prisma.$transaction?.(async (tx: any) => {
        ...
        await tx.job.update({
          where: { id: jobId },
          data:  { creditRefunded: true, status: 'REFUNDED' as any },
        });
      });
      logger?.info?.({ jobId, orgId: job.orgId, creditCost: job.creditCost }, '[crash-safety] Credits refunded atomically');
      return true;
    } catch (e: any) {
      logger?.error({ err: e.message, jobId }, '[crash-safety] protectCredits FAILED — manual review required');
      return false;
    }
  }
  ...
  async function sendToDeadLetter(
    ...
    try {
      ...
      logger?.error({ err: e.message, jobId }, '[crash-safety] sendToDeadLetter failed');
    }
  }
  ...
  async function recoverStuckJobs(stuckThresholdMs = 300_000): Promise<{
    ...
    try {
      const stuckJobs = await prisma.job?.findMany?.({
        where:  { status: { in: ['RUNNING' as any, 'PENDING' as any] }, startedAt: { lte: cutoff } },
      ...
      logger?.error({ err: e.message }, '[crash-safety] recoverStuckJobs error');
    }
  ...
  async function getDiagnostics(jobId: string): Promise<JobDiagnostics | null> {
    ...
    try {
      ...
      return {
        ...
        checkpointStage:      cp?.checkpointStage ?? null,
      ...
      logger?.warn({ err: e.message, jobId }, '[crash-safety] getDiagnostics failed');
      return null;
    }
  }
