...
function assertPaddleEnvironmentConsistency(event: PaddleEventBase): void {
  ...
  const actual = (('environment' in event ? (event as Record<string, unknown>).environment : 'sandbox') as string ?? 'sandbox').toLowerCase();
...
export async function handlePaddleEvent(
  ...
  switch (event.event_type) {
    ...
    case 'subscription.updated': {
      ...
      const billingPeriod = d.current_billing_period as Record<string, string> | undefined;
      const cycleStart = billingPeriod?.starts_at
        ? new Date(billingPeriod.starts_at) : undefined;
      const cycleEnd = billingPeriod?.ends_at
        ? new Date(billingPeriod.ends_at) : undefined;
    ...
    case 'transaction.completed': {
      ...
      if (planKey) {
        ...
        const creditAmount = ((d.custom_data as Record<string, unknown> | undefined)?.credits) as number | undefined;
        if (creditAmount && creditAmount > 0) {
          ...
          await auditSvc.log({
            ...
            targetId: org.id,
            metadata: { transactionId, credits: creditAmount },
          });
        }
      }
      break;
    ...
    case 'subscription.canceled': {
      ...
      if (sendEmail && org) {
        ...
        const email = (orgFull?.members as Array<{email: string}> | undefined)?.[0]?.email;
        if (email) {
          await sendEmail({
            to:      email,
