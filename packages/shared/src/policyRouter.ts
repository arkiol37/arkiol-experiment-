...
export async function persistRoutingPlan(
  ...
  jobId: string,
  orgId: string,
  deps: { prisma?: PrismaClient; logger?: { warn(o: unknown, m: string): void } }
  ...
  try {
    await deps.prisma.routingPlanLog?.create?.({
      data: {
        ...
        rationale:            toJsonValue(plan.rationale),
        routedAt:             new Date(plan.routedAt),
      },
    });
  } catch (e: unknown) {
    // Non-fatal: if RoutingPlanLog doesn't exist yet (pre-migration), log and continue
    deps.logger?.warn({ err: (e instanceof Error ? e.message : String(e)), planId: plan.planId }, '[policy-router] RoutingPlanLog write failed (non-fatal)');
  }
