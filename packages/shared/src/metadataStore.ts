...
export interface MetadataStoreDeps {
  /** Prisma client — typed as PrismaClient to prevent cross-package version conflicts */
  prisma?: PrismaClient;
...
export async function persistBenchmark(
  ...
  try {
    ...
    }).catch((e: Error) => logger?.warn({ err: (e instanceof Error ? e.message : String(e)) }, '[metadata] benchmark record write failed (non-fatal)'));

  } catch (e: unknown) {
    logger?.warn({ err: (e instanceof Error ? e.message : String(e)) }, '[metadata] persistBenchmark outer catch');
  }
  ...
  for (const [experimentName, variant] of Object.entries(benchmark.abVariants)) {
    upsertABResult(experimentName, variant as string, benchmark.orgId, score, benchmark.totalPipelineMs, prisma, logger).catch(() => {});
  }
}

async function upsertStylePerformance(
  stylePreset: string, orgId: string, score: number, pipelineMs: number,
  prisma: PrismaClient, logger?: MetadataStoreDeps['logger']
): Promise<void> {
  try {
    ...
    logger?.warn({ err: (e instanceof Error ? e.message : String(e)) }, '[metadata] style performance upsert failed (non-fatal)');
  }
}
...
async function upsertFormatPerformance(
  ...
  try {
    ...
    logger?.warn({ err: (e instanceof Error ? e.message : String(e)) }, '[metadata] format performance upsert failed (non-fatal)');
  }
}
...
async function upsertABResult(
  ...
  try {
    ...
    logger?.warn({ err: (e instanceof Error ? e.message : String(e)) }, '[metadata] A/B result upsert failed (non-fatal)');
  }
}
...
export async function persistJobSummary(
  ...
  try {
    ...
    logger?.warn({ err: (e instanceof Error ? e.message : String(e)) }, '[metadata] job summary upsert failed (non-fatal)');
  }
}
...
export async function getOrgPerformanceSnapshot(
  ...
  try {
    ...
    for (const r of abResults as Array<Record<string, unknown>>) {
      const experimentName = String(r['experimentName'] ?? '');
      const avgQualityScore = Number(r['avgQualityScore'] ?? 0);
      const variant = String(r['variant'] ?? '');
      const cur = abByExperiment[experimentName];
      if (!cur || avgQualityScore > cur.avgScore) {
        abByExperiment[experimentName] = { winner: variant, avgScore: avgQualityScore };
      }
    }

    const allScores = (jobs as Array<Record<string, unknown>>).map(j => Number(j['avgOverallScore'] ?? 0)).filter(Boolean);
    const globalAvg = allScores.length ? allScores.reduce((s: number, v: number) => s + v, 0) / allScores.length : 0;
    const totalCount = (jobs as Array<Record<string, unknown>>).reduce((s: number, j: Record<string, unknown>) => s + Number(j['assetCount'] ?? 0), 0);

    return {
      orgId,
      topStyles:   (styles as Array<Record<string, unknown>>).map(s => ({ style: String(s['stylePreset'] ?? ''), avgScore: Number(s['avgQualityScore'] ?? 0), trend: String(s['trend'] ?? '') })),
      worstFormats:(formats as Array<Record<string, unknown>>).map(f => ({ format: String(f['format'] ?? ''), avgScore: Number(f['avgQualityScore'] ?? 0), fallbackRate: Number(f['fallbackRate'] ?? 0) })),
    ...
    logger?.warn({ err: (e instanceof Error ? e.message : String(e)) }, '[metadata] getOrgPerformanceSnapshot failed');
    return null;
  }
}
