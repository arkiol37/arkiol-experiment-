...
async function lookupCache(
  ...
  try {
    ...
    console.warn('[asset-engine] Cache lookup failed (non-fatal):', err.message);
    return null;
  }
}
...
async function generateWithAI(
  ...
  if (assetType === 'photoreal' || assetType === 'illustrated') {
    ...
    const imgResp = await deps.openai.images.generate({
      model:   'dall-e-3',
    ...
    if (deps.uploadFn) {
      ...
      try {
        ...
        console.error(`[asset-engine] S3 upload failed for ${requestId}:`, uploadErr.message);
...
async function persistGeneratedAsset(
  ...
  try {
    ...
    await prisma.aIGeneratedAsset.upsert({
      where: { id: asset.id },
      create: {
        id:              asset.id,
        orgId,
    ...
    console.error('[asset-engine] DB persist failed (non-fatal):', err.message);
  }
}
...
export async function generateAssetOnDemand(
  ...
  if (!parsed.success) {
    errorResult(requestId, `Invalid request: ${parsed.error.errors.map(e => e.message).join('; ')}`, t0);
  }
  ...
  // ── Safety check ───────────────────────────────────────────────────────────
  const safety = await validateAssetSafety(missingEl.context, req.safetyLevel ?? 'strict');
  if (!safety.safe) {
