  ...
  try {
    await prisma.aIStageTrace?.create?.({
      data: {
        ...
        outputSummary:   toJsonValue(parsed.data.outputSummary ?? {}),
    ...
    logger?.warn({ err: (e instanceof Error ? e.message : String(e)), stageId: input.stageId }, '[stage-trace] Write failed (non-fatal)');
  }
}
  ...
  try {
    await prisma.aIJobMetadata?.upsert?.({
      ...
      create: {
        ...
        jobId:               d.jobId,
        orgId:               d.orgId,
        stageTimings:        toJsonValue(d.stageTimings        ?? {}),
        stageDecisions:      toJsonValue(d.stageDecisions      ?? {}),
        fallbackReasons:     toJsonValue(d.fallbackReasons     ?? []),
        abAssignments:       toJsonValue(d.abAssignments       ?? {}),
        stageOutputs:        toJsonValue(d.stageOutputs        ?? {}),
        costGateResults:     toJsonValue(d.costGateResults     ?? []),
        observabilityEvents: toJsonValue(d.observabilityEvents ?? []),
        ...
        fallbackTriggers:         toJsonValue(d.fallbackTriggers              ?? []),
      },
      update: {
        stageTimings:             toJsonValue(d.stageTimings                  ?? {}),
        stageDecisions:           toJsonValue(d.stageDecisions                ?? {}),
    ...
    logger?.warn({ err: (e instanceof Error ? e.message : String(e)), jobId: d.jobId }, '[stage-trace] Job metadata upsert failed (non-fatal)');
  }
}
