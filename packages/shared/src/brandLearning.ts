...
export async function isBrandLearningEnabled(
  ...
  try {
    ...
    deps.logger?.warn({ err: (e instanceof Error ? e.message : String(e)), orgId }, '[brand-learning] Flag check failed, defaulting to false');
    return false;
  }
}
...
export async function recordBrandSignal(
  ...
  try {
    ...
    deps.logger?.warn({ err: (e instanceof Error ? e.message : String(e)) }, '[brand-learning] Signal write failed (non-fatal)');
  }
}
...
export async function loadBrandLearningSnapshot(
  ...
  try {
    ...
    deps.logger?.warn({ err: (e instanceof Error ? e.message : String(e)), orgId }, '[brand-learning] Snapshot load failed, returning empty');
