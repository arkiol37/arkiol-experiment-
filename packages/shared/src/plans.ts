...
//   Free tier: 1 watermarked Normal Ad per day at no credit cost (teaser only).
...
//   Free plan receives a TEASER ONLY: 1 free watermarked Normal Ad per day.
//   Free users cannot access full Animation Studio workflows beyond this teaser.
//
//   Plan access summary:
//     FREE:    Teaser only — 1 free watermarked Normal Ad/day (no credits, no full Studio)
//     CREATOR: Full Animation Studio — Normal Ads (20 cr), Cinematic Ads (35 cr)
//     PRO:     Full Animation Studio — all modes, 4K export, batch, higher daily cap
//     STUDIO:  Full Animation Studio — all modes, highest limits, automation, team
//
...
export interface PlanConfig {
  ...
  canUseStudioVideo: boolean;    // full Animation Studio access (Creator, Pro, Studio only)
  ...
  maxDailyVideoJobs: number;     // 0 = fully blocked; FREE=1 (teaser only — 1 free watermarked Normal Ad)
  maxFormatsPerRun: number;
  maxVariationsPerRun: number;
  maxExportResolution: string;
  freeWatermarkEnabled: boolean;
  freeDailyNormalAds: number;    // teaser Normal Ad renders per day (watermarked, no credits deducted; FREE only)
  freeDailyCreditsPerDay: number; // daily credit bucket (0 for all plans — credits are monthly)
...
export const PLANS: Record<PlanKey, PlanConfig> = {
  FREE: {
    ...
    members: 1,
    brands: 1,
    // Free plan: NO full Animation Studio access. Teaser only.
    // The 1 free daily Normal Ad is gated by freeDailyNormalAds + watermark, not canUseStudioVideo.
    ...
    maxConcurrency: 1,
    queuePriority: 0,
    maxDailyVideoJobs: 1,          // teaser only: 1 free watermarked Normal Ad per day
    maxFormatsPerRun: 1,
    maxVariationsPerRun: 1,
    maxExportResolution: '1080p',
    freeWatermarkEnabled: true,
    freeDailyNormalAds: 1,         // 1 free watermarked Normal Ad/day; no credit deduction
  ...
  CREATOR: {
    ...
    members: 3,
    brands: 2,
    canUseStudioVideo: true,       // full Animation Studio access
...
//
// Free tier teaser:
//   FREE plan users get 1 free watermarked Normal Ad per day.
...
export function getTopupStripePriceId(packId: string): string | undefined {
  ...
  const map: Record<string, string | undefined> = {
    pack_200:  e.STRIPE_PRICE_TOPUP_200,
    pack_600:  e.STRIPE_PRICE_TOPUP_600,
    pack_2000: e.STRIPE_PRICE_TOPUP_2000,
  };
  return map[packId];
}
