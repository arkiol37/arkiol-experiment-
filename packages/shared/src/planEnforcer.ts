...
export function checkStudioVideoAccess(org: OrgEnforcementSnapshot): EnforcementResult {
  ...
  if (!plan.canUseStudioVideo) {
    return deny('Animation Studio video requires Pro or Studio plan.', 'PLAN_FEATURE_BLOCKED');
  }
  if (org.costProtectionBlocked) {
    return deny('Daily cost protection limit reached. Video jobs are blocked until tomorrow.', 'COST_PROTECTION_BLOCKED');
  }
  return ok;
}
...
export function checkGifAccess(org: OrgEnforcementSnapshot): EnforcementResult {
  ...
  if (!plan.canUseGifMotion) {
    return deny('GIF motion export requires Pro or Studio plan.', 'PLAN_FEATURE_BLOCKED');
  }
  return ok;
}
...
export function checkZipExport(org: OrgEnforcementSnapshot): EnforcementResult {
  ...
  if (!plan.canUseZipExport) {
    return deny('ZIP export requires Creator plan or higher.', 'PLAN_FEATURE_BLOCKED');
  }
  return ok;
}
...
export function checkBatchGenerate(org: OrgEnforcementSnapshot): EnforcementResult {
  ...
  if (!plan.canBatchGenerate) {
    return deny('Batch generation requires Pro or Studio plan.', 'PLAN_FEATURE_BLOCKED');
  }
  return ok;
}
...
export function checkBatchSize(org: OrgEnforcementSnapshot, requestedJobs: number): EnforcementResult {
  ...
  const planKey  = (getPlanConfig(org.plan) as any).key ?? org.plan.toUpperCase();
...
export function checkResolution(org: OrgEnforcementSnapshot, resolution: string): EnforcementResult {
  ...
  if (resolution === '4K' && plan.maxExportResolution !== '4K') {
    return deny('4K export requires Pro or Studio plan.', 'RESOLUTION_BLOCKED');
  }
  return ok;
}
...
export function checkCredits(
  ...
  // ── Free daily Normal Ad gate ──────────────────────────────────────────────
  // FREE plan: 1 Normal Ad per day at no credit cost (watermarked).
  ...
  if (isNormalAd && plan.freeDailyNormalAds > 0) {
    ...
    if (usedToday < plan.freeDailyNormalAds) {
      return ok; // Free daily Normal Ad — no credits deducted
    }
  }
...
export function preflightJob(params: {
  ...
  todayNormalAdCount?: number;   // for free-tier Normal Ad gate
  ...
  if (isVideoJob)  checks.push(checkStudioVideoAccess(org), checkDailyVideoJobs(org, todayVideoJobs));
...
export function checkHqUpgrade(org: OrgEnforcementSnapshot): EnforcementResult {
  ...
  return deny('HQ upgrade requires the Pro or Studio plan.', 'HQ_UPGRADE_NOT_ALLOWED', 403);
}
