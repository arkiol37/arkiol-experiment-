...
function assertStripeModeConsistency(event: Stripe.Event): void {
  ...
  const eventLive: boolean = (event as any).livemode === true;
...
export async function handleStripeEvent(event: Stripe.Event, deps: WebhookDeps): Promise<void> {
  ...
  switch (event.type) {
    ...
    case 'customer.subscription.updated': {
      ...
      await prisma.auditLog.create({
        data: {
          ...
          metadata: { plan: planKey, status: sub.status, event: event.type },
        },
      });
      break;
    }
    ...
    case 'customer.subscription.deleted': {
      ...
      await prisma.auditLog.create({
        data: { orgId: org.id, actorId: 'stripe', action: 'plan_change', metadata: { plan: 'FREE', event: event.type } },
      });
      break;
    }
    ...
    case 'invoice.paid': {
      ...
      if (!(invoice as any).subscription) break; // skip one-time invoices (handled by checkout.session.completed)
      ...
      await prisma.org.update({
        ...
        data: {
          ...
          currentCycleStart:     new Date((invoice as any).period_start * 1000),
          currentCycleEnd:       new Date((invoice as any).period_end   * 1000),
        },
      });
    ...
    case 'checkout.session.completed': {
      ...
      await prisma.auditLog.create({
        data: {
          ...
          metadata: { packId, credits: pack.credits, paymentIntentId },
        },
      });
      break;
    }
