    ...
    <div className="ak-card" style={{ padding: "14px 16px" }}>
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: engine.alerts.length > 0 ? 8 : 0 }}>
        {[
          ...
          <div key={m.label}>
            <p style={{ margin: 0, fontSize: 13, fontWeight: 400, fontFamily: "var(--font-display)", letterSpacing: "-0.04em" }}>{m.value}</p>
          ...
          {report.recentErrors.length === 0 ? (
            ...
            report.recentErrors.map((entry, i) => (
              <div key={i} className="ak-card" style={{ padding: "10px 14px", borderLeft: "3px solid var(--error)" }}>
                ...
                {typeof entry.ctx.stage === 'string' && entry.ctx.stage && (
                  <p style={{ margin: "2px 0 0", fontSize: 9, color: "var(--text-muted)" }}>Stage: {entry.ctx.stage}</p>
                )}
              </div>
            ))
          )}
