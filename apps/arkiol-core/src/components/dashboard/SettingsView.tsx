...
interface BillingData {
  ...
  credits: { limit: number; used: number; remaining: number; usagePct: number; };
  planLimits: { credits: number; members: number; brands: number; priceUsd: number; costPerCredit: number; };
...
export function SettingsView({ user }: { user: User }) {
  ...
  return (
    <div style={{ padding: "36px 44px", maxWidth: 760, fontFamily: "var(--font-body)" }}>
      <h1 style={{ margin: "0 0 28px", fontSize: 28, fontWeight: 800, color: "var(--text-primary)", letterSpacing: "-0.04em", fontFamily: "var(--font-display)" }}>
        Settings
      </h1>
              ...
              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 16 }}>
                <div>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
                    <span style={{ fontSize: 20, fontWeight: 800, letterSpacing: "-0.04em", fontFamily: "var(--font-display)", color: "var(--text-primary)" }}>
                ...
                {billing.allPlans.map(p => (
                  ...
                  >
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
                      <div>
                        <div style={{ fontSize: 15, fontWeight: 700, color: "var(--text-primary)", fontFamily: "var(--font-display)", letterSpacing: "-0.03em" }}>
                          {PLAN_LABELS[p.plan] ?? p.plan}
                        </div>
                        <div style={{ fontSize: 22, fontWeight: 800, color: "var(--text-primary)", letterSpacing: "-0.05em", fontFamily: "var(--font-display)", marginTop: 4 }}>
                          ${p.priceUsd}
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 14 }}>
                      <PlanFeature label={`${p.credits.toLocaleString()} credits`} />
                      {p.members > 2 && <PlanFeature label={`${p.members === 999 ? "Unlimited" : p.members} members`} />}
                      <PlanFeature label={`${p.brands === 999 ? "Unlimited" : p.brands} brand${p.brands !== 1 ? "s" : ""}`} />
                    </div>
                    {!p.currentPlan && (
                      <button
