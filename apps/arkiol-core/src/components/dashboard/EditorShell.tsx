...
function FormatStep({ selected, onSelect }: { selected: string; onSelect: (f: string) => void }) {
  return (
    <div style={{ padding: "40px 40px", maxWidth: 900 }}>
      <h2 style={{ margin: "0 0 6px", fontSize: 24, fontWeight: 800, color: "var(--text-primary)", fontFamily: "var(--font-display)", letterSpacing: "-0.03em" }}>Choose a format</h2>
      <p style={{ margin: "0 0 28px", color: "var(--text-muted)", fontSize: 14 }}>Pick the canvas you want to design for</p>
...
function BriefStep({ format, prompt, setPrompt, style, setStyle, vars, setVars, ytMode, setYtMode, error, onBack, onGenerate }: any) {
  ...
  return (
    <div style={{ padding: "40px 40px", maxWidth: 640 }}>
      ...
      <h2 style={{ margin: "0 0 6px", fontSize: 24, fontWeight: 800, color: "var(--text-primary)", fontFamily: "var(--font-display)", letterSpacing: "-0.03em" }}>Describe your design</h2>
      ...
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 18 }}>
        {STYLES.map(s => (
          <button key={s} onClick={() => setStyle(s)} style={{
            ...
            border: style === s ? "1px solid rgba(99,102,241,0.4)" : "1px solid rgba(255,255,255,0.08)",
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 18 }}>
        {[1,2,3].map(n => (
          <button key={n} onClick={() => setVars(n)} style={{
            ...
            border: vars === n ? "1px solid rgba(99,102,241,0.4)" : "1px solid rgba(255,255,255,0.08)",
          }}>{n}</button>
        ))}
      </div>
          ...
          <div style={{ display: "flex", gap: 6, marginBottom: 18 }}>
            {["auto","face","product"].map(m => (
              <button key={m} onClick={() => setYtMode(m)} style={{
                ...
                border: ytMode === m ? "1px solid rgba(99,102,241,0.4)" : "1px solid rgba(255,255,255,0.08)",
              }}>{m}</button>
            ))}
          </div>
        </>
