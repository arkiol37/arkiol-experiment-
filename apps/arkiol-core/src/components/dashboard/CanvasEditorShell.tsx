...
interface Cfg { label:string; w:number; h:number; bg:string; }
    ...
    return (
      <div style={{ height:"100vh", display:"flex", flexDirection:"column" }}>
        <div style={{ height:44, display:"flex", alignItems:"center", gap:12, padding:"0 16px", borderBottom:"1px solid var(--border)", background:"var(--bg-surface)", flexShrink:0, zIndex:200 }}>
          ...
          <div style={{ flex:1 }}/>
          <span style={{ fontSize:11, color:"var(--accent-light)", background:"var(--accent-tint)", padding:"2px 10px", borderRadius:"var(--radius-full)", border:"1px solid var(--border-accent)" }}>Manual Editor</span>
        </div>
        <div style={{ flex:1, overflow:"hidden" }}>
          <ArkiolEditor
            key={`canvas-${cfg.w}-${cfg.h}-${Date.now()}`}
            projectId={`canvas_${Date.now()}`}
            initialElements={[]}
            canvasWidth={cfg.w}
            canvasHeight={cfg.h}
            canvasBg={cfg.bg}
    ...
    <div className="ak-fade-in" style={{ padding:"40px 48px", maxWidth:1080 }}>
      <div style={{ marginBottom:32 }}>
        <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:8 }}>
          <h1 style={{ margin:0, fontSize:28, fontWeight:800, fontFamily:"var(--font-display)", letterSpacing:"-0.045em" }}>Arkiol Canvas</h1>
