"use client";
// AnimationStudioView — Normal Ads (2D) + Cinematic Ads (2.5D)
// Plan-gated: canUseStudioVideo (Creator, Pro, Studio)

import React, { useState, useEffect, useRef } from "react";

const MODES = [
  { id: "Normal Ad",   tag: "2D",   credits: 20, icon: "◫", color: "#4F46E5", badge: "Standard",
    desc: "Smooth 2D animated ad. Perfect for social feeds and direct-response campaigns.",
    features: ["Smooth 2D motion","Typography animation","Brand color integration","Loop-ready output"] },
  { id: "Cinematic Ad",tag: "2.5D", credits: 35, icon: "✦", color: "#7c3aed", badge: "Premium",
    desc: "Premium cinematic quality with parallax depth and advanced lighting.",
    features: ["2.5D parallax depth","Cinematic camera motion","Advanced lighting","4K quality export"] },
] as const;
...
export function AnimationStudioView() {
  ...
  useEffect(() => {
    fetch("/api/capabilities").then(r=>r.json()).then(d=>{ if(d.canUseStudioVideo===false) setGated(true); }).catch(()=>{});
  }, []);
  ...
  if (gated) return (
    <div className="ak-fade-in" style={{ padding:"60px 48px", maxWidth:680, margin:"0 auto", textAlign:"center" }}>
      ...
      <h1 style={{ fontSize:28, fontWeight:800, fontFamily:"var(--font-display)", letterSpacing:"-0.04em", margin:"0 0 12px" }}>Animation Studio</h1>
      <p style={{ fontSize:14, color:"var(--text-secondary)", lineHeight:1.7, marginBottom:32 }}>
        Generate Normal Ads (2D) and Cinematic Ads (2.5D) from a single prompt.<br/>Available on <strong>Creator, Pro, and Studio</strong> plans.
      </p>
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:32, textAlign:"left" }}>
        {[["◫","Normal Ads (2D)","Smooth 2D animations — 20 credits"],["✦","Cinematic Ads (2.5D)","Parallax depth & lighting — 35 credits"],["🎨","Auto brand integration","Applies your brand kit"],["📐","All ad formats","Posts, stories, banners & more"]].map(([icon,label,desc])=>(
  ...
  return (
    <div className="ak-fade-in" style={{ padding:"36px 44px", maxWidth:1200 }}>
      <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:32 }}>
        <div>
          <h1 style={{ margin:0, fontSize:28, fontWeight:800, fontFamily:"var(--font-display)", letterSpacing:"-0.045em" }}>🎬 Animation Studio</h1>
          <p style={{ margin:"5px 0 0", fontSize:13, color:"var(--text-secondary)" }}>Generate Normal Ads (2D) or Cinematic Ads (2.5D) from a single prompt</p>
        </div>
              ...
              <div style={{ display:"flex", gap:10 }}>
                {DURATIONS.map(d=>(
                  <button key={d.v} onClick={()=>setDur(d.v)} disabled={genning} style={{ flex:1, padding:"12px 8px", borderRadius:"var(--radius-md)", border:`1px solid ${dur===d.v?"var(--border-accent)":"var(--border-strong)"}`, background:dur===d.v?"var(--accent-tint-md)":"transparent", cursor:"pointer", fontFamily:"var(--font-body)", textAlign:"center" }}>
                    <div style={{ fontSize:18, fontWeight:800, color:dur===d.v?"var(--accent-light)":"var(--text-primary)", fontFamily:"var(--font-display)", letterSpacing:"-0.03em" }}>{d.l}</div>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:14, position:"sticky", top:20 }}>
            ...
            <div style={{ background:"var(--accent-tint-md)", border:"1px solid var(--border-accent)", borderRadius:"var(--radius-lg)", padding:"12px 18px", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div>
                ...
                <div style={{ fontSize:11, color:"var(--text-muted)" }}>{mode==="Cinematic Ad"?"Premium 2.5D rendering":"Standard 2D rendering"}</div>
              </div>
              <div style={{ display:"flex", alignItems:"baseline", gap:4 }}>
                <span style={{ fontSize:28, fontWeight:800, fontFamily:"var(--font-display)", color:"var(--accent-light)", letterSpacing:"-0.05em" }}>{sm.credits}</span>
        ...
        <div>
          ...
          {libLoad ? (
            ...
            <div style={{ textAlign:"center", padding:"60px 24px", background:"var(--bg-elevated)", borderRadius:"var(--radius-2xl)", border:"1px dashed var(--border-strong)" }}>
              ...
              <h3 style={{ fontSize:18, fontWeight:800, fontFamily:"var(--font-display)", margin:"0 0 8px" }}>No video ads yet</h3>
              ...
              {assets.map(a=>(
                <div key={a.id} className="ak-gallery-card">
                  <div style={{ aspectRatio:"16/9", background:"var(--bg-overlay)", position:"relative", overflow:"hidden" }}>
                    ...
                    <div style={{ position:"absolute", top:8, left:8 }}><span style={{ background:"rgba(0,0,0,.7)", color:"#fff", fontSize:9.5, fontWeight:700, padding:"2px 8px", borderRadius:99, backdropFilter:"blur(4px)" }}>{a.renderMode==="Cinematic Ad"?"2.5D":"2D"}</span></div>
