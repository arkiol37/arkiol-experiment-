  ...
  return (
    <div style={{ position: "relative", width: size, height: size, flexShrink: 0 }}>
      ...
      <div style={{
        ...
        <span style={{ fontSize: size * 0.22, fontWeight: 800, fontFamily: "var(--font-display)", color, letterSpacing: "-0.04em" }}>
          {score}
        </span>
      </div>
    </div>
...
function DesignCard({ asset, selected, onSelect, isExperimental }: {
  asset: Asset; selected: boolean; onSelect: () => void; isExperimental?: boolean;
}) {
    ...
    >
      <div style={{
        ...
        <div style={{
          ...
          <button className="ak-btn ak-btn-ghost ak-btn-sm" style={{ fontSize: 11, color: "rgba(255,255,255,0.6)" }} onClick={e => { e.stopPropagation(); onSelect?.(); }}>
            ✦ Generate Similar
          </button>
        </div>
        {selected && (
  ...
  return (
    <div className="ak-fade-in" style={{ padding: "36px 44px", maxWidth: 1440 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28 }}>
        <div>
          <h1 style={{ margin: 0, fontSize: 28, fontWeight: 800, fontFamily: "var(--font-display)", letterSpacing: "-0.045em" }}>Arkiol Art</h1>
      ...
      {loading && assets.length === 0 ? (
        ...
        <div style={{ textAlign: "center", padding: "80px 24px", background: "var(--bg-elevated)", borderRadius: "var(--radius-2xl)", border: "1px dashed rgba(124,127,250,0.2)" }}>
          ...
          <h3 style={{ fontSize: 20, fontWeight: 800, fontFamily: "var(--font-display)", margin: "0 0 10px", letterSpacing: "-0.04em" }}>No designs yet</h3>
              ...
              <div style={gridStyle}>
                {highConfidence.map((asset, i) => (
                  <div key={asset.id} style={{ animation: `ak-slide-up-stagger ${280 + i * 40}ms ease both`, animationDelay: `${i * 30}ms` }}>
                    <DesignCard asset={asset} selected={selected.has(asset.id)} onSelect={() => toggleSelect(asset.id)} />
                  </div>
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <div style={gridStyle}>
                {experimental.map((asset, i) => (
                  <div key={asset.id} style={{ animation: `ak-slide-up-stagger ${280 + i * 40}ms ease both`, animationDelay: `${i * 30}ms` }}>
                    <DesignCard asset={asset} selected={selected.has(asset.id)} onSelect={() => toggleSelect(asset.id)} isExperimental />
                  </div>
                ))}
              </div>
            </div>
