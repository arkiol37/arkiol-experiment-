...
export const GET = withErrorHandling(async (
  ...
  // Batch status is stored in DB — no queue required for polling
  ...
  interface BatchJobRow {
    id: string; status: string; progress: number | null; result: Record<string, unknown> | null;
    createdAt: Date; startedAt: Date | null; completedAt: Date | null; failedAt: Date | null;
  }

  const jobs: BatchJobRow[] = jobIds.length > 0
