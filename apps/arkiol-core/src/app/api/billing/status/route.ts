...
// Web reads:    full shape including subscriptionStatus, hasActivePaddle, canUseStudioVideo
//
// canUseStudioVideo is included in ALL response shapes so that AnimationStudioView
// and other plan-gated components can read it from this endpoint without needing
// to call the heavier /api/billing endpoint.
...
export const GET = withErrorHandling(async (req: NextRequest) => {
  ...
  if (isFounderEmail(_statusEmail) || user.role === "SUPER_ADMIN") {
    return NextResponse.json({
      ...
      // Plan-gating flags — always true for founder
  ...
  if (!dbUser.org) {
    ...
    return NextResponse.json({
      ...
      currentCycleEnd:    null,
      hasActivePaddle:    false,
      // Plan-gating: FREE plan cannot access full Animation Studio
  ...
  return NextResponse.json({
    ...
    // Plan-gating flag — read from org row (set by Stripe/Paddle webhook on plan change)
