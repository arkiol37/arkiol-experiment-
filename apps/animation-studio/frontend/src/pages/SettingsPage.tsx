          ...
          {TABS.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)}
              className={`w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all
                ${tab===t.id ? 'bg-ink-700 text-teal-300' : 'text-ink-200 hover:bg-ink-800 hover:text-ink-50'}`}>
...
function BillingTab({ workspace, navigate }: any) {
  ...
  const COLORS: Record<string,string> = { free:'#94a3b8', creator:'#14B8A6', pro:'#f59e0b', studio:'#10b981' };
        ...
        <div className="flex items-center gap-4 p-4 bg-ink-800 rounded-xl mb-5" style={{ borderLeft:`3px solid ${c}` }}>
          ...
          <div className="flex-1">
            ...
            <div className="text-xs text-ink-300 mt-1">
              <span className="font-bold text-teal-300">{workspace?.creditsBalance?.toLocaleString()||0}</span> credits remaining
            </div>
          </div>
          {workspace?.plan!=='free' && (
        ...
        {isLoading ? (
          ...
          <div key={inv.id} className="flex items-center justify-between py-2.5 px-3 rounded-lg hover:bg-ink-800 transition-colors">
            ...
            <div className="flex items-center gap-3">
              ...
              {inv.pdf && <a href={inv.pdf} target="_blank" rel="noreferrer" className="text-[10px] text-teal-400 hover:underline">PDF</a>}
            </div>
          </div>
        ))}
      </div>
      ...
      <AnimatePresence>
        {dirty && (
          <motion.div initial={{ opacity:0,y:12 }} animate={{ opacity:1,y:0 }} exit={{ opacity:0,y:12 }}
            ...
            style={{ background:'rgba(15,118,110,0.1)',border:'1px solid rgba(15,118,110,0.3)' }}>
...
function SecurityTab({ user, show }: any) {
  ...
  const sColor = ['','#ef4444','#f59e0b','#14B8A6','#10b981','#10b981'][s]||'';
    ...
    <div className="space-y-4">
      ...
      <div className="card p-6">
        <div className="flex items-center gap-3 mb-5">
          <div style={{ width:32,height:32,borderRadius:9,background:'rgba(15,118,110,0.15)',display:'flex',alignItems:'center',justifyContent:'center' }}>
            <Key size={15} color="#2DD4BF"/>
          </div>
          <div>
...
const RENDER_DEFAULTS = [
  ...
  { key:'default_render_mode',  label:'Default Render Mode',  options:['Normal Ad','Cinematic Ad'] },
...
function DefaultsTab({ show }: { show:(t:string,k?:'success'|'error')=>void }) {
  const [prefs, setPrefs] = useState<Record<string,any>>({
    ...
    default_aspect_ratio:'9:16', default_render_mode:'Normal Ad',
