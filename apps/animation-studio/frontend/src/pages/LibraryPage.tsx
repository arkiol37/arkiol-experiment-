...
function typeColor(type: string) {
  const colors: Record<string, string> = {
    logo: '#f59e0b', product: '#14B8A6', video: '#ef4444',
    ...
    <div className="p-8 min-h-screen">
      ...
      <div
        ...
        className={`mb-5 border-2 border-dashed rounded-2xl p-8 text-center cursor-pointer transition-all duration-200
          ${isDragActive ? 'border-teal-500 bg-teal-600/5 scale-[1.01]' : 'border-white/10 hover:border-white/20 hover:bg-white/[0.02]'}`}
      >
        <input {...getInputProps()} />
        <CloudUpload size={28} className={`mx-auto mb-2.5 transition-colors ${isDragActive ? 'text-teal-400' : 'text-ink-400'}`} />
          ...
          >
            {uploads.map(u => (
              <div key={u.id} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                {u.status === 'uploading' && <Loader2 size={13} className="animate-spin text-teal-400 flex-shrink-0" />}
                ...
                {u.status === 'uploading' && (
                  <div style={{ width: 80, height: 3, borderRadius: 99, background: 'rgba(255,255,255,0.08)', flexShrink: 0 }}>
                    <div style={{ height: '100%', borderRadius: 99, width: `${u.progress}%`, background: '#14B8A6', transition: 'width 0.2s' }} />
                  </div>
                )}
          ...
          {TYPES.map(t => (
            <button
              ...
              style={{
                ...
                background: type === t.value ? 'rgba(15,118,110,0.15)' : 'rgba(255,255,255,0.05)',
                color: type === t.value ? '#14B8A6' : 'var(--text-muted)',
                border: `1px solid ${type === t.value ? 'rgba(15,118,110,0.35)' : 'rgba(255,255,255,0.07)'}`,
              }}
            >
              {t.icon} {t.label}
            </button>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-6 gap-3">
                ...
                >
                  <div style={{ display: 'flex', gap: 6 }}>
                    <button
                      ...
                      style={{ padding: '6px 12px', borderRadius: 8, background: '#14B8A6', border: 'none', color: '#0B0F10', fontSize: 11, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}
                    >
                      <ZoomIn size={11} /> View
                    </button>
                    <button
          ...
          <div
            ...
            onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = 'rgba(15,118,110,0.4)'; (e.currentTarget as HTMLElement).style.background = 'rgba(15,118,110,0.04)'; }}
