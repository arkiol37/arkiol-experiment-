...
import { projectsApi, rendersApi, brandAssetsApi, engineApi } from '../lib/api';
...
// Normal Ads: 20 credits  |  Cinematic Ads: 35 credits
// FREE tier: 1 free watermarked Normal Ad per day (no credits deducted)
const CREDIT_COSTS: Record<string, number> = {
  'Normal Ad':   20,   // launch mode
  'Cinematic Ad': 35,  // launch mode
};
...
export default function StudioPage() {
  ...
  const [engineStoryboard, setEngineStoryboard] = useState<any[]>([]);
  const [engineTimeline, setEngineTimeline] = useState<any[]>([]);
  const [engineShotPlans, setEngineShotPlans] = useState<any[]>([]);
  const [engineViolations, setEngineViolations] = useState<any[]>([]);
  const [engineQualityIssues, setEngineQualityIssues] = useState<any[]>([]);
  const [engineAudioSync, setEngineAudioSync] = useState<any[]>([]);
  const [engineMusicProfile, setEngineMusicProfile] = useState<any>(null);
  const [activeSceneId, setActiveSceneId] = useState<string | undefined>();
  const [renderStatus, setRenderStatus] = useState('idle');
  const [renderProgress, setRenderProgress] = useState(0);
  const [renderVideoUrl, setRenderVideoUrl] = useState<string | undefined>();
  ...
  // ── AI Engine: Generate Creative Preview ─────────────────────────────
  const handleEnginePreview = useCallback(async () => {
    if (!selectedPlacement || !brand.name) return;
    try {
      const result = await engineApi.generateAnimation({
        brief: brand.brief || `Ad for ${brand.name}`,
        brandName: brand.name,
        ...
        maxDurationSec: selectedPlacement.maxSec,
        objective,
        brandAssetIds: selectedBrandAssets,
        brandPalette: brandAssetPalette,
      });
      if (result.success) {
        ...
        if (result.animaticPreview) setEnginePreview((prev: any) => ({ ...prev, animaticPreview: result.animaticPreview }));
        // Map storyboard scenes for preview components
        if (result.storyboard) setEngineStoryboard(result.storyboard);
        if (result.stages) {
          const qIssues = result.stages.filter((s: any) => s.name === 'quality_scoring' && s.status === 'complete');
          if (qIssues.length > 0) setEngineQualityIssues([]);
        }
      }
    } catch (err) {
      console.warn('Engine preview failed (non-blocking):', err);
    }
  }, [selectedPlacement, brand, mood, hookType, selectedPlatform, selectedPlacementId, sceneCount, aspectRatio, effectiveRenderMode, objective]);

  // Auto-trigger engine preview when key inputs change
  useEffect(() => {
    if (step >= 4 && brand.name && selectedPlacement) {
      const timeout = setTimeout(handleEnginePreview, 800);
      return () => clearTimeout(timeout);
    }
  }, [step, brand.name, mood, hookType, sceneCount, selectedPlacementId]);
  ...
  return (
    <div className="min-h-screen p-8">
      ...
      {/* ── Free plan paywall banner — full Animation Studio requires Creator, Pro, or Studio ── */}
      {(() => {
        const plan = workspace?.plan?.toUpperCase() ?? 'FREE';
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <span className="text-xs font-semibold" style={{ color: '#14B8A6' }}>
                Free plan — teaser mode
              </span>
              <span className="text-xs text-ink-400 ml-2">
                You have 1 free watermarked Normal Ad per day. Upgrade to Creator, Pro, or Studio for full Animation Studio access.
              </span>
            </div>
            <a
              href="/pricing"
        ...
        <div>
          ...
          <p className="page-subtitle">
            {selectedPlacement
              ...
              : 'AI Director builds your ad video in 5 steps'}
          </p>
        </div>
            ...
            <div key={s} className="flex items-center gap-1">
              <button onClick={() => s < step && setStep(s)}
                className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-all
                  ${s === step ? 'bg-teal-600 text-ink-900' : s < step ? 'bg-teal-600/30 text-teal-300 hover:bg-teal-600/40 cursor-pointer' : 'bg-ink-700 text-ink-300 cursor-default'}`}>
                {s}
              </button>
              {s < TOTAL_STEPS && <div className={`w-6 h-0.5 rounded ${s < step ? 'bg-teal-600/50' : 'bg-ink-700'}`} />}
            </div>
          ))}
        </div>
      </div>
          ...
          <motion.div key={step}
            ...
            {step === 1 && (
              <div className="card p-6 space-y-6">
                <div className="flex items-center gap-3 pb-4 border-b border-white/[0.06]">
                  <div className="w-9 h-9 rounded-xl bg-teal-600/15 flex items-center justify-center text-lg">🏢</div>
              ...
              <div className="card p-6 space-y-6">
                  ...
                  <div className="grid grid-cols-5 gap-2">
                    {MOODS.map(m => (
                      <button key={m} onClick={() => setMood(m)}
                        className={`flex flex-col items-center gap-1.5 p-3 rounded-xl border transition-all ${mood === m ? 'bg-teal-600/15 border-teal-600/40 text-teal-300' : 'bg-ink-800 border-white/[0.06] text-ink-200 hover:border-white/15'}`}>
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      >
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="text-[10px] text-ink-400 leading-snug mt-0.5">Smooth motion graphics. Optimised for volume.</div>
                        </div>
                        {adStyle === 'normal' && <div className="absolute top-2 right-2 w-1.5 h-1.5 rounded-full bg-green-400" />}
                      </button>
                      {/* Cinematic Ad Option — Creator, Pro, Studio only */}
                      {(() => {
                        const plan = workspace?.plan?.toUpperCase() ?? 'FREE';
                        // Cinematic Ads require Creator or higher (Normal Ads = 20 cr, Cinematic = 35 cr)
                        const canUseCinematic = plan === 'CREATOR' || plan === 'PRO' || plan === 'STUDIO';
                        return (
                          <button
                            onClick={() => {
                              if (canUseCinematic) {
                                setAdStyle('cinematic');
                                setRenderMode('Cinematic Ad');
                              } else {
                                // Free plan: redirect to upgrade — no access to Cinematic Ads
                                window.location.href = '/pricing';
                              }
                            }}
                            className={`relative flex items-start gap-3 p-3 rounded-xl border transition-all text-left ${adStyle === 'cinematic' ? 'bg-teal-600/10 border-teal-600/40 text-teal-100' : 'bg-ink-800 border-white/[0.06] text-ink-300 hover:border-teal-600/20'}`}
                          >
                            <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 text-sm mt-0.5 ${adStyle === 'cinematic' ? 'bg-teal-600/20' : 'bg-white/[0.04]'}`}>🎬</div>
                            <div className="min-w-0">
                              <div className="text-xs font-bold flex items-center gap-1.5">
                                Cinematic Ad
                                <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-teal-600/20 text-teal-300 uppercase tracking-wide">Premium</span>
                                {!canUseCinematic && (
                                  <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-red-500/20 text-red-400 uppercase tracking-wide">Upgrade</span>
                                )}
                              </div>
                              <div className="text-[10px] text-ink-400 leading-snug mt-0.5">Parallax depth · camera moves · cinematic grade</div>
                              {!canUseCinematic && (
                                <div className="text-[9px] text-amber-400 mt-1 font-semibold">
                                  Requires Creator plan or above — click to upgrade
                                </div>
                              )}
                            </div>
                            {adStyle === 'cinematic' && <div className="absolute top-2 right-2 w-1.5 h-1.5 rounded-full bg-teal-600" />}
                          </button>
                        );
                      })()}
                    </div>
                  ...
                  <div>
                    {adStyle === 'normal' ? (
                      ...
                      <div>
                        ...
                        <div className="space-y-1.5">
                          {['Parallax depth layers', 'Camera movement', 'Depth-of-field blur', 'Film color grade', 'Atmospheric vignette'].map(f => (
                            <div key={f} className="flex items-center gap-2 text-[10px] text-teal-200">
                              <div className="w-3 h-3 rounded-full bg-teal-600/30 flex items-center justify-center flex-shrink-0">
                    ...
                    <div className="flex gap-2">
                      {(['1080p','4K'] as const).map(r => (
                        <button key={r} onClick={() => setResolution(r)}
                          className={`flex-1 py-2 text-xs font-bold rounded-xl border transition-all ${resolution === r ? 'bg-teal-600/15 border-teal-600/40 text-teal-300' : 'bg-ink-800 border-white/[0.06] text-ink-300 hover:border-white/15'}`}>
                          {r}{r === '4K' && ' +5cr'}
                        </button>
                      ))}
                    </div>
                ...
                <div>
                  <label className="form-label flex justify-between">
                    Scenes
                    <span className="text-teal-300 font-mono">{sceneCount} · {sceneCount * (selectedPlacement?.secPerScene || 7)}s{selectedPlacement ? ` / ${selectedPlacement.maxSec}s max` : ''}</span>
                  </label>
            ...
            {step === 4 && (
              <div className="card p-6 space-y-6">
                  ...
                  {scenes.map((s, i) => (
                    <div key={i} className="p-3 bg-ink-800 rounded-xl border border-white/[0.06]">
                      <div className="flex items-center gap-2 mb-2">
                        <span className="px-2 py-0.5 bg-teal-600/15 text-teal-300 text-[10px] font-bold rounded-full uppercase">{s.role}</span>
                ...
                {/* AI Engine Panels */}
                {engineStoryboard.length > 0 && (
                  <StoryboardPreview scenes={engineStoryboard} activeSceneId={activeSceneId} onSceneClick={setActiveSceneId} />
                )}
                {engineQualityIssues.length > 0 && (
                  <SceneWarnings issues={engineQualityIssues} />
                )}
                <div className="flex justify-between pt-2">
                ...
                <div className="flex items-center gap-3 pb-4 border-b border-white/[0.06]">
                  ...
                  {selectedBrandAssets.length > 0 && (
                    <div className="ml-auto flex items-center gap-2">
                      <span className="px-2 py-0.5 bg-teal-600/15 text-teal-300 text-[10px] font-bold rounded-full">
                  ...
                  <div className="space-y-1.5">
                    ...
                    {assetSlots.map((slot: any) => (
                      <div key={slot.sceneRole} className="flex items-center gap-3 p-2.5 bg-ink-800 rounded-xl border border-white/[0.06]">
                        <span className="px-2 py-0.5 bg-teal-600/15 text-teal-300 text-[9px] font-bold rounded-full uppercase">{slot.sceneRole}</span>
            ...
            {step === 6 && (
              <div className="card p-6 space-y-6">
                      ...
                      <div className="flex gap-2">
                        {['Male','Female','Neutral'].map(g => (
                          <button key={g} onClick={() => setVoice(v => ({ ...v, gender: g }))}
                            className={`flex-1 py-2 text-xs font-semibold rounded-xl border transition-all ${voice.gender === g ? 'bg-teal-600/15 border-teal-600/40 text-teal-300' : 'bg-ink-800 border-white/[0.06] text-ink-300 hover:border-white/15'}`}>{g}</button>
                        ))}
                      </div>
                    </div>
                    <div>
                      ...
                      <div className="flex gap-2">
                        {['Slow','Normal','Fast','Very Fast'].map(sp => (
                          <button key={sp} onClick={() => setVoice(v => ({ ...v, speed: sp }))}
                            className={`flex-1 py-1.5 text-[10px] font-bold rounded-xl border transition-all ${voice.speed === sp ? 'bg-teal-600/15 border-teal-600/40 text-teal-300' : 'bg-ink-800 border-white/[0.06] text-ink-300 hover:border-white/15'}`}>{sp}</button>
                        ))}
                      </div>
                    </div>
                  </div>
                      ...
                      <div className="space-y-1.5">
                        {MUSIC_STYLES.map(s => (
                          <button key={s} onClick={() => setMusic(m => ({ ...m, style: s }))}
                            className={`w-full text-left px-3 py-2 rounded-xl text-xs font-semibold border transition-all ${music.style === s ? 'bg-teal-600/15 border-teal-600/40 text-teal-300' : 'bg-ink-800 border-white/[0.06] text-ink-200 hover:border-white/15'}`}>{s}</button>
                        ))}
                      </div>
                    </div>
                    <div>
                ...
                <div className="flex justify-between pt-2">
                  ...
                  <button onClick={handleGenerate} className="btn btn-primary px-8 py-3 text-sm shadow-teal-lg">
        ...
        <div className="space-y-4">
          <div className="card p-4">
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs font-bold text-teal-300 uppercase tracking-wide">Ad Preview</span>
            ...
            <div className={`relative bg-ink-800 rounded-xl overflow-hidden ${previewAspect[aspectRatio]} max-h-64 flex items-center justify-center border border-white/[0.06]`}>
              ...
              <div className="relative z-10 text-center p-4">
                <div className="text-xs font-black text-teal-300 mb-1 uppercase tracking-widest">{brand.name || 'Your Brand'}</div>
                ...
                {ctaText && <div className="mt-3 inline-block px-3 py-1.5 bg-teal-600 text-ink-900 text-[10px] font-black rounded-lg">{ctaText}</div>}
              </div>
            </div>
            {sceneCount > 0 && (
              <div className="flex gap-1 mt-3">
                {Array.from({ length: sceneCount }).map((_, i) => (
                  <div key={i} className={`flex-1 h-1 rounded-full ${i === 0 ? 'bg-teal-600' : 'bg-ink-600'}`} />
                ))}
              </div>
            )}
          </div>
          ...
          <div className="card p-4">
            ...
            <div className="text-xs text-ink-400 mb-3">{sceneCount} scenes · {adStyle === 'cinematic' ? '🎬 Cinematic Ad' : '⚡ Normal Ad'} · {selectedPlacement?.label || aspectRatio} · {resolution}</div>
            <div className="space-y-1.5 border-t border-white/[0.06] pt-3">
              <div className="flex justify-between text-xs"><span className={`${adStyle === 'cinematic' ? 'text-teal-300' : 'text-ink-300'}`}>{adStyle === 'cinematic' ? '🎬 Cinematic Ad' : '⚡ Normal Ad'}</span><span className="font-mono text-ink-100">{totalCredits} cr</span></div>
            ...
            {workspace && workspace.creditsBalance < totalCredits && !(adStyle === 'normal' && (workspace.plan === 'FREE' || workspace.plan === 'free')) && (
              <div className="mt-3 p-2 bg-red-500/10 border border-red-500/25 rounded-xl text-xs text-red-400 font-medium">
                ⚠ Need {totalCredits - workspace.creditsBalance} more credits to generate
              </div>
            )}
          </div>
          ...
          {enginePreview && (
            <div className="card p-4 space-y-3">
              <div className="text-[10px] font-semibold text-teal-300 uppercase tracking-widest mb-1">AI Engine Preview</div>
              {enginePreview.confidence !== undefined && (
                <div className="flex items-center gap-2">
                  <div className="flex-1 h-1.5 bg-ink-700 rounded-full overflow-hidden">
                    <div className="h-full bg-teal-500 rounded-full" style={{ width: `${Math.round(enginePreview.confidence * 100)}%` }} />
                  </div>
                  <span className="text-[10px] font-mono text-teal-400">{Math.round(enginePreview.confidence * 100)}%</span>
                </div>
              )}
              {enginePreview.frameContinuity && (
                <div className="flex items-center gap-2 text-[10px] mb-2">
                  <span className="text-ink-400">Continuity:</span>
                  <span className={enginePreview.frameContinuity.avgScore >= 80 ? 'text-green-400' : 'text-yellow-400'}>{enginePreview.frameContinuity.avgScore}/100</span>
                  {enginePreview.frameContinuity.violations > 0 && <span className="text-orange-400">({enginePreview.frameContinuity.violations} issues)</span>}
                </div>
              )}
              {enginePreview.performances && enginePreview.performances.length > 0 && (
                <div className="text-[10px] text-ink-400 mb-2">
                  Acting: {enginePreview.performances.filter((p: any) => p.productActing).length} product scenes, {enginePreview.performances.length} performances
                </div>
              )}
              {enginePreview.cinematicDirection && (
                <div className="text-[10px] text-ink-400 mb-2">
                  Cinematic: {enginePreview.cinematicDirection.shots?.length || 0} directed shots
                </div>
              )}
              {enginePreview.engineValidation && (
                <div className="flex items-center gap-2 text-[10px] mb-2">
                  <span className="text-ink-400">Engines:</span>
                  <span className={enginePreview.engineValidation.avgScore >= 70 ? 'text-green-400' : 'text-yellow-400'}>
                    {Math.round(enginePreview.engineValidation.avgScore)}/100
                  </span>
                  <span className="text-ink-500">({enginePreview.engineValidation.totalDirectives} directives)</span>
                </div>
              )}
              {enginePreview.stages && (
                <div className="space-y-0.5">
                  {enginePreview.stages.slice(0, 6).map((s: any, i: number) => (
                    <div key={i} className="flex items-center gap-2 text-[10px]">
                      <span className={s.status === 'complete' ? 'text-green-400' : s.status === 'failed' ? 'text-red-400' : 'text-ink-400'}>
                        {s.status === 'complete' ? '✓' : s.status === 'failed' ? '✗' : '○'}
                      </span>
                      <span className="text-ink-300">{s.name.replace(/_/g, ' ')}</span>
                      {s.durationMs && <span className="ml-auto text-ink-500 font-mono">{s.durationMs}ms</span>}
                    </div>
                  ))}
                </div>
              )}
          ...
          {/* Animatic Preview */}
          {enginePreview?.animaticPreview && (
            <div className="card p-4 space-y-2">
              <div className="text-[10px] font-semibold text-teal-300 uppercase tracking-widest">Animatic Preview</div>
              <div className="flex gap-1">
                {enginePreview.animaticPreview.scenes?.map((s: any, i: number) => (
                  <div key={i} className="flex-1 rounded-md p-1.5 text-center" style={{ background: s.dominantColor + '20', borderBottom: `2px solid ${s.dominantColor}` }}>
                    <div className="text-[8px] text-white/60 uppercase">{s.role}</div>
                    <div className="text-[10px] font-mono text-white/40">{(s.durationMs/1000).toFixed(0)}s</div>
                  </div>
                ))}
              </div>
              <div className="flex justify-between text-[10px] text-ink-400">
                <span>Quality: {enginePreview.animaticPreview.qualityPrediction}/100</span>
                <span>Est: {enginePreview.animaticPreview.estimatedRenderTimeSec}s</span>
              </div>
            </div>
          )}

          {/* Render Preview Player */}
          <RenderPreviewPlayer videoUrl={renderVideoUrl} status={renderStatus} progress={renderProgress} renderJobId="" />
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
              ...
              <div className="relative w-20 h-20 mx-auto mb-6">
                <div className="absolute inset-0 rounded-full border-2 border-teal-600/20 border-t-teal-400 animate-spin" />
                <div className="absolute inset-2 rounded-full border-2 border-teal-600/10 border-b-teal-400/60 animate-[spin_2s_linear_infinite_reverse]" />
                <div className="absolute inset-4 rounded-full bg-teal-600/10 flex items-center justify-center text-2xl">🎬</div>
              </div>
              ...
              <span className="font-mono text-xs text-teal-400">{genProgress}%</span>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
...
// ── AI Engine Studio Components ──────────────────────────────────────────
import DirectorControls from '../components/studio/DirectorControls';
import StoryboardPreview from '../components/studio/StoryboardPreview';
import ShotPlanPreview from '../components/studio/ShotPlanPreview';
import TimelinePreview from '../components/studio/TimelinePreview';
import ContinuityPanel from '../components/studio/ContinuityPanel';
import SceneWarnings from '../components/studio/SceneWarnings';
import AudioSyncControls from '../components/studio/AudioSyncControls';
import RenderPreviewPlayer from '../components/studio/RenderPreviewPlayer';
