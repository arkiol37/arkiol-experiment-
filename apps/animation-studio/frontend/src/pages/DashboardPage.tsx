...
export default function DashboardPage() {
  ...
  const stats = [
    { label: 'Credits Balance', value: workspace?.creditsBalance ?? 0, change: '', icon: Coins, accent: 'from-teal-700 to-teal-400' },
    ...
    { label: 'Avg Scenes', value: analytics?.renderStats?.avg_scenes ? Number(analytics.renderStats.avg_scenes).toFixed(1) : '—', change: '', icon: Zap, accent: 'from-emerald-700 to-emerald-500' },
  ];
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="page-title">
            ...
            <span className="text-teal-300">{user?.firstName}</span> ✦
          </h1>
          <p className="page-subtitle">Here's what's happening with your campaigns</p>
        </div>
        <button onClick={() => navigate('/studio')} className="btn btn-primary gap-2 px-5 py-2.5 text-sm shadow-teal">
          <Plus size={15} />
          New Campaign
        </button>
      </div>
        ...
        <div className="col-span-2">
          <div className="card">
            <div className="flex items-center justify-between p-5 border-b border-white/[0.06]">
              ...
              <button onClick={() => navigate('/projects')} className="text-xs text-teal-400 hover:text-teal-300 transition-colors font-semibold">
                View all →
              </button>
            </div>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        {['queued','processing','scene_rendering','mixing'].includes(job.status) && (
                          <div className="text-xs font-mono text-teal-400">{job.progress}%</div>
                        )}
                    ...
                    {job.status === 'complete' && hasPlatformExports && (
                      <div className="flex flex-wrap gap-1.5 mt-3 pl-16">
                        ...
                        <button
                          ...
                          className="flex items-center gap-1.5 px-2.5 py-1 bg-teal-500/15 hover:bg-teal-500/25 border border-teal-500/30 rounded-lg text-[10px] font-semibold text-teal-300 transition-all"
            ...
            <div className="card p-5">
              ...
              <div className="mb-3">
                <div className="flex justify-between text-xs mb-1.5">
                  ...
                  <span className="font-mono font-bold text-teal-300">{workspace.creditsBalance}</span>
                </div>
