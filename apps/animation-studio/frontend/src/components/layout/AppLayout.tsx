...
// ── Arkiol animated mark (emerald/teal gradient) ─────────────────────────────
    ...
    <>
      ...
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} fill="none" style={{ overflow:"visible", flexShrink:0 }}>
        ...
        <circle cx={cx} cy={cy} r={rO} stroke="url(#ak2-arc)" strokeWidth={size*0.028}
          strokeDasharray={`${rO*4.5} ${rO*1.6}`} strokeLinecap="round" opacity={0.7}
          filter="url(#ak2-glow)"
    ...
    <div className="flex min-h-screen">
      ...
      <aside className="sidebar">
        ...
        <div className="flex items-center gap-3 mb-8 px-1">
          ...
          <div>
            <div style={{ fontFamily: "'Syne','Helvetica Neue',sans-serif", fontWeight: 800, fontSize: 15, letterSpacing: "-0.6px", color: "#E5E7EB" }}>arkiol</div>
            <div style={{ fontFamily: "'Syne','Helvetica Neue',sans-serif", fontSize: 9, letterSpacing: "0.25em", color: "#14B8A6", opacity: 0.7 }}>DESIGN INTELLIGENCE</div>
          </div>
        </div>
        ...
        {workspace && (
          <div className="mb-6 px-1">
            ...
            <div className="flex items-center gap-2 p-2 bg-ink-600 rounded-xl" style={{ border: '1px solid rgba(255,255,255,0.05)' }}>
              <div className="w-7 h-7 rounded-lg flex items-center justify-center text-xs font-bold" style={{ background: 'rgba(15,118,110,0.2)', color: '#14B8A6' }}>
                {workspace.name[0]}
              </div>
        ...
        {/* Credits */}
        {workspace && (
          <div className="mb-6 mx-1 p-3 bg-ink-700 rounded-xl" style={{ border: '1px solid rgba(255,255,255,0.05)' }}>
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-1.5 text-[10px] font-semibold text-ink-200 uppercase tracking-widest">
                <Coins size={10} style={{ color: '#14B8A6' }} />
                Credits
              </div>
              <span className="font-mono text-xs font-bold" style={{ color: '#14B8A6' }}>{workspace.creditsBalance}</span>
            </div>
        ...
        <nav className="flex-1 space-y-0.5">
          {NAV_ITEMS.map(({ to, label, icon: Icon, primary }) => (
            <NavLink
              ...
              to={to}
              className={({ isActive }) =>
                `nav-item ${isActive ? 'active' : ''} ${primary ? 'mt-2' : ''}`
              }
              style={({ isActive }) => primary && !isActive ? {
                color: '#14B8A6',
        ...
        <div className="mt-6 space-y-0.5 pt-4" style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
          ...
          {user && (
            <div className="relative">
              ...
              >
                <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0" style={{ background: 'rgba(15,118,110,0.2)', color: '#14B8A6' }}>
              ...
              {showUserMenu && (
                <motion.div
                  ...
                  className="absolute bottom-full left-0 right-0 mb-2 bg-ink-700 rounded-xl overflow-hidden shadow-xl z-50"
