...
describe('enhancePrompt — render mode: Normal Ad vs Cinematic Ad', () => {
  ...
  it('Cinematic Ad uses cinematic depth language', () => {
    // enhancePrompt output for Cinematic Ad: "Multi-layer 2.5D depth composition, parallax depth, cinematic lighting..."
    // 3D is not a launch mode — Cinematic Ad is 2.5D parallax. Match actual output words.
    const result = enhancePrompt('test', cfg({ renderMode: 'Cinematic Ad' }));
    expect(result.toLowerCase()).toMatch(/2\.5d|parallax|cinematic/i);
  });
...
describe('estimateGpuCost — cost ordering', () => {
  it('Cinematic Ad has higher GPU cost than Normal Ad', () => {
    // Launch modes: Normal Ad (2D) and Cinematic Ad (2.5D).
    // 3D is not a launch mode — film3d test removed.
    const scenes = 3;
    const costs = {
      normal:    estimateGpuCost('Normal Ad', scenes),
      cinematic: estimateGpuCost('Cinematic Ad', scenes),
    };
    expect(costs.cinematic).toBeGreaterThan(costs.normal);
  });
});
