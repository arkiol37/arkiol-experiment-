...
try {
  // eslint-disable-next-line @typescript-eslint/no-var-requires
  const shared = require('@arkiol/shared');
  if (shared && typeof shared.validateSharedEnv === 'function') {
    shared.validateSharedEnv();
    console.log('✅ [animation-studio] Shared environment validation passed.');
  } else {
    console.warn('⚠️  [animation-studio] Shared env validation skipped (function not found — run npm run build in packages/shared)');
  }
} catch (err: any) {
  // Shared package may not be built yet — warn but don't crash
  if (err.code === 'MODULE_NOT_FOUND' || err.message?.includes('Cannot find module')) {
    console.warn('⚠️  [animation-studio] Shared package not available — skipping shared env validation');
    console.warn('   Run: npm run build --workspace=packages/shared');
  } else {
